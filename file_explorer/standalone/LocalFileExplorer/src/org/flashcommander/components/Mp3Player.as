/*
http://flashcommander.org/flashcommander
*/
package org.flashcommander.components
{
	...
	import org.flashcommander.components.Mp3PlayerSkin;
	...
	public class Mp3Player extends SkinnableContainer
	{
		...
		private function errorHandler(event:IOErrorEvent){
		...
		private function playSound(event){
			if (isPlaying){
				pause();
		...
		private function handleTime(event:TimerEvent){
			if (!isPlaying) return;
			soundPosition = myChannel.position;
			updateDisplay()
			updateScrubBar()
